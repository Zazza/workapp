...
namespace Otms\Modules\Photo\Ajax;
 ...
 * Ajax\Photo class
 *
 * Основной для Photo класс, т.к. все действия в нём вызываются AJAX
 *
 * @author Dmitry Samotoy <dmitry.samotoy@gmail.com>
 */
class Photo extends Ajax {
	/**
	 * Настройки модуля
	 *
	 * @var array
    ...
    if (isset($this->registry["post"]["did"])) {
    	...
    	$this->_mfile = new Model\Photofile($config);
    	$this->_mphoto = new Model\Photo();
    }
    ...
    function files() {
		...
		if ( (isset($fm["admin"])) and ($fm["admin"]) ) {
			if ( ($sess) and (!$fav) ) {
				$files = $this->_mfile->getAdminFilesSort();
			} else if ( ($sess) and ($fav) ) {
				$files = $this->_mfile->getAdminFilesFavorite();
			} else {
				$files = $this->_mfile->getAdminFiles($curdir);
			}
		} else {
			if ( ($sess) and (!$fav) ) {
				$files = $this->_mfile->getFilesSort();
			} else if ( ($sess) and ($fav) ) {
				$files = $this->_mfile->getFilesFavorite();
			} else {
        ...
        return $this->view->render("fm_content", array("admin" => $fm["admin"], "clip" => $clip, "shPath" => $shPath, "dirs" => $res_dirs, "_thumb" => $this->registry['path']['photo'] . "_thumb/", "files" => $res_files, "totalsize" => $total, "curdir" => $this->_curdir, "fsRes" => $this->view->render("fm_view", array())));
    }
    ...
    function copyFiles($params) {
        ...
        if (isset($params["file"])) {
	    	...
	    	foreach($buffer["files"] as $part) {
	    		...
	    		$res = true;
	    		$content = "<li><span class='buf_file' title='" . $part . "'>" . $shortname . "</span></li>";
	    	}
        } else {
        	$buffer["files"] = "0";
        }
        if (isset($params["dir"])) {
        	...
        	foreach($buffer["dirs"] as $part) {
    			...
    			$res = true;
    			$part = $this->_dir["name"];
    			$content = "<li><span class='buf_folder' title='" . $part . "'>" .  $shortname . "</span></li>";
        	}
        } else {
        	$buffer["dirs"] = "0";
        }
        ...
        if (!$res) {
        	$content = "<li style='text-align: center'>empty</li>";
        }
    ...
    /**
     * Добавить выделение участка на фотографии с описанием
     * 
     * @param array
     *    $params["x1"]
     *    $params["x2"]
     *    $params["y1"]
     *    $params["y2"]
     *    $params["ws"] - отношение между оригинальным размером изображения и показанным
     *    $params["desc"] - текстовое описание
     *    $params["md5"]
     * @return string (twig render) - .../fm/desc.tpl
     */
    public function setDesc($params) {
    	...
    	return $this->view->render("fm_desc", array("desc" => $desc, "tags" => $tags));
    }
    
    /**
     * Получить все выделенные области на фотографии и их описания
     * @param array
     *    $params["md5"]
     * @return string (twig render) - .../fm/desc.tpl
     */
    ...
    /**
     * Получить количество текстовых записей к фотографии
     * 
     * @param array
     *    $params["id"]
     * @return int
    ...
    public function getNumNotes($params) {
    	...
    	return $this->_mphoto->getNumNotes($md5);
    }
    
    /**
     * Получить текстовые записи к фотографии
     * 
     * @param array
     *    $params["id"]
     * @return string (twig render) - .../fm/ftext.tpl
    ...
    /**
     * Добавить к сессии сортировки - название выбранного участка
     * 
     * @param array
     *    $params["word"]
     */
    ...
    /**
     * Добавить к сессии сортировки - тег
     * 
     * @param array
     *    $params["word"]
     */
    ...
    /**
     * Удалить из сессии сортировки - название выбранного участка
     * @param array
     *    $params["word"]
     */
    ...
    /**
     * Удалить из сессии сортировки - тег
     * @param array
     *    $params["word"]
     */
    ...
    public function favorite($params) {
    	...
    	if ($this->_mphoto->favorite($md5)) {
    		return "<img src='" . $this->registry["uri"] . "img/add.png' alt='' /> Image add in favorite list";
    	} else {
    		return "<img src='" . $this->registry["uri"] . "img/remove.png' alt='' /> Image delete from favorite list";
    	}
    }
    
    /**
     * Добавить к сессии сортировки избранное
     */
    ...
    /**
     * 
     *  Удалить из сессии сортировки избранное
     */
