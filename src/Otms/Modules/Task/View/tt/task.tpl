	...
	<div style="font-size: 12px; padding-bottom: 3px;">
		...
		<li class="dropdown">
			...
			<ul class="dropdown-menu">
				...
				<p class="chtask">
					<a style="color: #048; text-decoration: underline" href="{{ registry.uri }}task/show/{{ part.chid }}/">№{{ part.chid }}</a>
					{{ part.chname }}
				</p>
				</li>
				{% endfor %}
...
<div style="float: right; text-align: right">
	...
	{% if type != "draft" %}
	<a class="title" style="color: #048" href="{{ registry.uri }}task/show/{{ data.0.id }}/">№{{ data.0.id }}</a>
	{% else %}
	<a class="title" style="color: #048" href="{{ registry.uri }}task/draft/{{ data.0.id }}/">№{{ data.0.id }}</a>
	{% endif %}
...
{% if type != "draft" %}
{% if data.0.remote_id != 0 %}
<div style="margin: 4px; float: left"><code><a class="attach" style="margin-right: 10px; cursor: pointer" href="{{ registry.uri }}task/attach/?remote=1&tid={{ data.0.id }}&filename={{ part.filename }}">{{ part.filename }}</a></code></div>
{% else %}
<div style="margin: 4px; float: left"><code><a class="attach" style="margin-right: 10px; cursor: pointer" href="{{ registry.uri }}task/attach/?tid={{ data.0.id }}&filename={{ part.filename }}">{{ part.filename }}</a></code></div>
{% endif %}
{% else %}
<div style="margin: 4px; float: left"><code><a class="attach" style="margin-right: 10px; cursor: pointer" href="{{ registry.uri }}task/attach/?did={{ data.0.id }}&filename={{ part.filename }}">{{ part.filename }}</a></code></div>
{% endif %}
{% endfor %}
</div>
{% endif %}
...
{% if data.0.mail_id == 0 %}
{% if data.0.name %}
<div class="title"><a class="title" style="color: #048" href="{{ registry.uri }}task/show/{{ data.0.id }}/">{{ data.0.name }}</a></div>
{% endif %}
{% else %}
...
<div style="overflow: hidden; clear: both">
	...
	<a href="{{ registry.uri }}task/show/{{ data.0.id }}/">
