...
{% if data.0.remote_id == 0 %}
	<b>Автор задачи:</b> {% if data.0.remote_id == 0 %}<a style="cursor: pointer;" onclick="getUserInfo('{{ author.uid }}')">{% endif %}{{ author.soname }} {{ author.name }}{% if data.0.remote_id == 0 %}</a>{% endif %}
{% else %}
	<b>Автор задачи:</b> {% if data.0.remote_id == 0 %}<a style="cursor: pointer;" onclick="getUserInfo('{{ author.uid }}')">{% endif %}{{ author.soname }} {{ author.name }} (группа {{ author.gname }}){% if data.0.remote_id == 0 %}</a>{% endif %}
{% endif %}
</div>
</div>
