...
{% for part in mail %}
{% set i = i + 1 %}
<a style="cursor: pointer; text-decoration: none" onclick="showText('{{ mail.0.id }}', '{{ i }}')"><span class="btn btn-mini">{{ part.type }}</span></a>
{% endfor %}
</div>
...
<div style="float: right">
	<a class="btn btn-mini"  href="{{ registry.uri }}mail/sort/?mid={{ mail.0.id }}">
	...
	<a class="btn btn-mini" href="{{ registry.uri }}mail/compose/?action=reply&mid={{ mail.0.id }}">
		<i class="icon-repeat"></i>
		answer
	</a>
	
	{% if not isset %}
	<a class="btn btn-mini" onclick="addTaskFromMail('{{ mail.0.id }}')">
		<i class="icon-plus"></i>
		create task
	</a>
	{% endif %}
