...
<div style="text-align: center;"><h5>{% if gid == 0 %} Задачи без группы {% else %} {{ group[gid] }} {% endif %}</h5></div>
	...
	<div style="float: left; width: 55px; text-align: center; font-size: 12px; background-color: #EEE; border: 1px solid #6696AF; height: 22px; margin-right: 5px; padding-top: 3px;"><a class="tttip" href="{{ registry.uri }}task/show/{{ arrtask }}/" rel="tooltip" data-placement="top" data-original-title="{{ task[arrtask]|e }}">№{{ arrtask }}</a></div>
