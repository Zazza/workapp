<?php
class Model_Mail extends Modules_Model {
	public $mbox;
	private $contact;
	...
	function getMails() {
		...
		$row = $res->fetchAll(PDO::FETCH_ASSOC);
/*		
	    $sql = "SELECT mail.id, mail.uidl, mail.read AS `read`, mail.to AS `to`, mail.subject AS `subject`, MAX(mail.date) AS `date`, mail.timestamp AS `timestamp`, mail.personal AS `personal`, mail.email AS `email`
        FROM mail
        WHERE mail.uid = :uid AND mail.status = '0'
        GROUP BY mail.email ASC
        ORDER BY `date` DESC";
		...
		$sql = "SELECT mo.id AS id, mo.to AS `to`, mo.subject AS `subject`, MAX(mo.timestamp) AS `date`, mo.email AS `email`
		FROM mail_out AS mo
		WHERE mo.uid = :uid
		GROUP BY mo.to ASC
		ORDER BY `date` DESC";
