...
<div class="fm_unsellabel">
	<div ondblClick="chdir('{{ part.id }}')"><img src="{{ registry.uri }}{{ registry.path.modules }}FM/img/ftypes/folder.png" style="width: 50px{{ opacity }}" alt="[DIR]" /></div>
...
<div class="fm_dirs_up" title="{{ part.name }}" style="text-align: center; cursor: pointer">
	<div ondblClick="chdir('{{ part.pid }}')"><img src="{{ registry.uri }}{{ registry.path.modules }}FM/img/ftypes/folder.png" style="width: 50px{{ opacity }}" alt="[DIR]" /></div>
...
$(".fm_file").contextMenu('fileMenu', {
    bindings: {
      'rm_open': function(t) {
		window.location.href = "{{ registry.uri }}fm/attach/?did={{ curdir }}&filename=" + encodeURIComponent(t.title);
      },
...
$(".fm_pre").live("dblclick", function(){
	window.location.href = "{{ registry.uri }}fm/attach/?did={{ curdir }}&filename=" + encodeURIComponent($(this).attr("name"));
});
</script>
